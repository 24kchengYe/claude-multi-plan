...
description: 在任意终端（PowerShell/Git Bash/WSL）用快捷命令在 Claude 官方套餐、Kimi 套餐与 DeepSeek 套餐之间切换运行 Claude Code，互不影响登录态；提供 cd* Codex CLI 后端矩阵（cdx 当前槽位、cdds DeepSeek、cdkm Kimi）；并提供字节内部链路快捷命令（ta/trae=TRAE CLI、ccta=Trae CN 网关、ccta-aiden/ccad=Aiden AIProxy）。当用户想配置多套餐切换、在不同设备复用同样配置、或问 cc/cckm/cckimi/ccds/cdx/cdkm/cdds/ta/trae/ccta/ccta-aiden 之类命令时使用。
---
...
| `cdx` | Codex CLI × 当前 cc-switch 槽位 | 全权限 |
| `cdds` | Codex CLI × DeepSeek | 全权限（deepseek-v4-pro，self key，隔离 CODEX_HOME） |
| `cdkm` | Codex CLI × Kimi | 全权限（k3，KIMI_CODE_API_KEY，隔离 CODEX_HOME） |
...
> `cd*` = Codex CLI 的后端矩阵（全权限）：`cdx` 当前 cc-switch 槽位、`cdds` DeepSeek、`cdkm` Kimi。后端 key 从 ai-api-gateway 解锁文件读取，用隔离 CODEX_HOME（`~/.codex-deepseek` / `~/.codex-kimi`），不碰 cc-switch 登录态。
> `cd` 本身是 shell 换目录命令不能占用，所以当前槽位用 `cdx`。Claude Code 侧的全权限版就是 `cckm`/`ccds` 本身（默认已带 `--dangerously-skip-permissions`），无需别名。
...
- Model：`deepseek-v4-pro[1m]`（**默认主力**，思考型旗舰；`[1m]` 后缀开启 1M 上下文档位，不带后缀端点按 200k 标准档服务）；小任务/后台用 `deepseek-chat`（V4 Flash，快且便宜）
- Small/Fast：`ANTHROPIC_SMALL_FAST_MODEL=deepseek-chat`（后台小任务/标题生成走 haiku 档，用 Flash 避免 404 且省钱）
- 各档位映射：FABLE/OPUS/SONNET 与 `CLAUDE_CODE_SUBAGENT_MODEL` → `deepseek-v4-pro[1m]`；HAIKU/SMALL_FAST → `deepseek-chat`
...
> 与 Kimi 不同：DeepSeek 不需要 `CLAUDE_CODE_EFFORT_LEVEL`（思考与否由模型名决定：`deepseek-v4-pro` 思考、`deepseek-chat` 非思考）。1M 上下文靠模型名后缀 `[1m]` 开启（`deepseek-v4-pro[1m]`），不要只设 `CLAUDE_CODE_MAX_CONTEXT_TOKENS` 来"骗"显示——不带后缀时 DeepSeek 端点按 200k 标准档服务，Claude Code 状态栏会显示 200k。
